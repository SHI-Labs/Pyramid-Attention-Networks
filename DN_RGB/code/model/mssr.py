...
class MultisourceProjection(nn.Module):
    def __init__(self, in_channel,kernel_size = 3, conv=common.default_conv):
        ...
        self.up_attention = ContextualAttention()
...
class RecurrentProjection(nn.Module):
    def __init__(self, in_channel,kernel_size = 3, conv=common.default_conv):
        ...
        self.multi_source_projection = MultisourceProjection(in_channel,kernel_size=kernel_size,conv=conv)
        ...
        self.error_encode = nn.Sequential(*[nn.ConvTranspose2d(in_channel,in_channel,6,stride=2,padding=2),nn.PReLU()])
    ...
    def forward(self, x):
        ...
        x_final = self.down_sample_2(h_estimate)
