function PP_Evaluate_CAR_Y_PSNR_SSIM()
...
for idx_method = 1:num_method
    ...
    for idx_set = 1%:num_set
        ...
        for sigma = sigma_current %[10, 20, 30, 40]
            filepaths = [];
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile(path_HQ, dataset{idx_set}, ['Q', num2str(sigma)], ext{idx_ext})));
            end
            ...
            for idx_im = 1:length(filepaths)
                ...
                im_HR = imread(fullfile(path_HQ, dataset{idx_set}, ['Q', num2str(sigma)], name_HR));
                im_SR = imread(fullfile([methods{idx_method}, '/results'], dataset{idx_set}, ['Q', num2str(sigma)], name_SR));
                if size(im_SR, 3) == 3
                    im_SR = im_SR(:,:,1);
                end
                im_HR = double(im_HR);
